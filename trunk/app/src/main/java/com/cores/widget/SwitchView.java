...
public class SwitchView extends View {
		...
		if ((state == STATE_SWITCH_ON || state == STATE_SWITCH_OFF) && (sAnim * bAnim == 0)) {
			switch (event.getAction()) {
				...
				case MotionEvent.ACTION_UP:
					lastState = state;
					if (state == STATE_SWITCH_OFF) {
						refreshState(STATE_SWITCH_OFF2);//open
					} else if (state == STATE_SWITCH_ON) {
						refreshState(STATE_SWITCH_ON2);//close
					}
					bAnim = 1;
					invalidate();
	 ...
	 * @param isOpened
	 */
	public void toggleSwitchNotAnim(final boolean isOpened) {

		if(isOpened)
		{
			state = STATE_SWITCH_OFF;
			refreshState(STATE_SWITCH_ON);//open
		}
		else
		{
			state = STATE_SWITCH_ON;
			refreshState(STATE_SWITCH_OFF);//close
		}
		bAnim=1;
	}
